#!/bin/bash
# qwen-agentworld-mtp-apex-server.sh — Qwen-AgentWorld-35B-A3B MTP APEX-I-Quality runner
#
# Uses SC117/Qwen-AgentWorld-35B-A3B-MTP-APEX-GGUF with APEX-I-Quality quantization.
# MoE model (35B-A3B) with draft-mtp for ~1.15-1.25x speedup.
# Imatrix-calibrated for high quality at lower bit-rates compared to standard quantizations.
#
# === MODES ===
# --coding   : Thinking mode, precise coding/agentic tasks
#              temp=0.6, top_p=0.95, top_k=20, min_p=0.0, presence_penalty=0.0
# --agentic  : Non-thinking mode (instruct), for tool-calling/world-model work
...
MODEL_DIR="${MODEL_DIR:-$HOME/models/SC117/Qwen-AgentWorld-35B-A3B-MTP-APEX-GGUF}"
...
# Model paths
MODEL="${MODEL_DIR}/Qwen-AgentWorld-35B-A3B-MTP-APEX-I-Quality.gguf"
...
# If no use case specified, default to --agentic (since it's an AgentWorld model)
if [ -z "$USE_CASE" ]; then
    USE_CASE="agentic"
    ENABLE_THINKING=0
    TEMP=0.7
...
# === Print config ===
echo "=== Qwen-AgentWorld-35B-A3B MTP APEX Runner ==="
