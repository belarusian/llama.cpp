...
resolve_alias() {
    case "$1" in
        qwen3-coder-next|qwen3-coder-next-q8_k|qwen3-coder-next-q8) echo "${MODEL_DIR}/bartowski/qwen3-coder-next-q8/Qwen_Qwen3-Coder-Next-Q8_0/Qwen_Qwen3-Coder-Next-Q8_0-00001-of-00003.gguf" ;;
        qwen3.6-27b|qwen3.6-27b-q4_k_m) echo "${MODEL_DIR}/bartowski/qwen3.6-27b-q4/Qwen_Qwen3.6-27B-Q4_K_M.gguf" ;;
        qwen3.6-27b-q8|qwen3.6-27b-q8_0) echo "${MODEL_DIR}/bartowski/qwen3.6-27b-q8/Qwen_Qwen3.6-27B-Q8_0.gguf" ;;
        qwen3.6-27b-bf16) echo "${MODEL_DIR}/bartowski/qwen3.6-27b-bf16/Qwen_Qwen3.6-27B-bf16/Qwen_Qwen3.6-27B-bf16-00001-of-00002.gguf" ;;
...
resolve_alias_extra() {
    case "$1" in
        qwen3-coder-next|qwen3-coder-next-q8_k|qwen3-coder-next-q8)
            echo "-np 1 -fa on -ngl 99 --jinja -c 262144 --reasoning off"
            ;;
        qwen3.6-27b|qwen3.6-27b-q4_k_m)
            echo "--mmproj ${MODEL_DIR}/bartowski/mmproj/mmproj-Qwen_Qwen3.6-27B-f16.gguf -np 1 -fa on -ngl 99 --jinja -c 262144 --reasoning off"
            ;;
...
if [ $# -lt 1 ]; then
    ...
    echo "  qwen3-coder-next         Qwen3-Coder-Next (Q8_0, bartowski) — ~85GB (also: qwen3-coder-next-q8_k, qwen3-coder-next-q8)"
    echo "  qwen3.6-27b              Qwen3.6-27B (Q4_K_M, bartowski) — ~18GB (also: qwen3.6-27b-q4_k_m)"
    echo "  qwen3.6-27b-q8           Qwen3.6-27B (Q8_0, bartowski) — ~29GB (also: qwen3.6-27b-q8_0)"
    echo "  qwen3.6-27b-bf16         Qwen3.6-27B (BF16, bartowski) — ~55GB"
    ...
    echo "  gpt-oss-20b              GPT-OSS-20B (auto-download) - GPU optimized with -np 1"
    echo "  gpt-oss-120b             GPT-OSS-120B (auto-download) - GPU optimized with -np 1"
    echo ""
    echo "GPT-OSS models support --reasoning-effort (low|medium|high)"
    echo ""
    echo "Examples:"
    echo "  $0 qwen3-coder-next --port 8082"
    echo "  $0 qwen3.6-27b-q8 --port 8092"
    echo "  $0 qwen3.6-27b-bf16 --port 8094"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --reasoning-effort)
            case "$2" in
                 low|minimal)
                     EXTRA_FLAGS="${EXTRA_FLAGS} --reasoning low"
                     ;;
                 medium|default)
                     EXTRA_FLAGS="${EXTRA_FLAGS} --reasoning medium"
                     ;;
                 high)
                     EXTRA_FLAGS="${EXTRA_FLAGS} --reasoning high"
                     ;;
                 *)
...
case "$MODEL_SPEC" in
    gpt-oss-20b)
        ...
        echo "model:  gpt-oss-20b (auto-download)"
         echo "port:   $PORT"
         echo "config: -np 1 -fa on -ngl 99 -ctk q8_0 -ctv q8_0 --jinja -c 262144"
         echo ""
         exec "$LLAMA_SERVER" \
             --gpt-oss-20b-default \
            --host 0.0.0.0 \
